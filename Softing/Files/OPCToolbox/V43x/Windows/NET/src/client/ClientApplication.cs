			...
			try{				
				...
				{					
					...
					for(int i = 0; i< count; i++)
					{
						...
						{							
							object myData = (uint)Marshal.GetObjectForNativeVariant((IntPtr)((uint) pEvents[i].m_pEventAttrs + ValueQT.VARIANT_SIZE * j));
		...
		{	
			try{
				...
				if(session != null)
				{
					...
					{						
						object eventAttributte = (object)Marshal.GetObjectForNativeVariant((IntPtr)((uint)pConditionState.m_pEventAttrs + ValueQT.VARIANT_SIZE * i));

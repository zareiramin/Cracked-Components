	...
	public class InfoCollector : System.Windows.Forms.Form {
		...
		private uint RemoveItemsFromSelectedSubscription(DaAddressSpaceElement anAddressSpaceElement) {
			try {
				...
				DaAddressSpaceElement[] addressSpaceElements = null;
                if (ResultCode.SUCCEEDED(m_session.Browse(anAddressSpaceElement.ItemId, anAddressSpaceElement.ItemPath, browseOptions, out addressSpaceElements, m_executionOptions)))
                {
					for (int i = 0; i < addressSpaceElements.Length; i++) {
						string itemID = addressSpaceElements[i].ItemId;
		...
		private bool AddressSpaceElementHasItems(DaAddressSpaceElement anAddressSpaceElement){
			try{
                ...
                m_session.Browse(anAddressSpaceElement.ItemId, anAddressSpaceElement.ItemPath, browseOptions, out addressSpaceElements, options);
		...
		private void RefreshBrowseItemsTreeView(){
			...
			try{
				...
				if ((m_session.CurrentState == EnumObjectState.CONNECTED)||(m_session.CurrentState == EnumObjectState.ACTIVATED)) {
					...
					DaAddressSpaceElement root = new DaAddressSpaceElement(EnumAddressSpaceElementType.BRANCH, String.Empty, String.Empty, String.Empty, IntPtr.Zero, null);
					root.Name = m_session.Url;
					treeRoot.Tag = root;
					DaAddressSpaceElementBrowseOptions browseOptions = new DaAddressSpaceElementBrowseOptions();
					browseOptions.ElementTypeFilter = EnumAddressSpaceElementType.BRANCH;
					DaAddressSpaceElement[] addressSpaceElements = null;
                    if (ResultCode.SUCCEEDED(m_session.Browse(String.Empty, String.Empty, browseOptions, out addressSpaceElements, m_executionOptions)))
                    {
						for (int i = 0; i < addressSpaceElements.Length; i++) {
			...
			try{
				...
				if (e.Node.Nodes != null) {
					foreach (TreeNode rootNode in e.Node.Nodes) {
						...
						browseOptions.ElementTypeFilter = EnumAddressSpaceElementType.BRANCH;
						DaAddressSpaceElement[] addressSpaceElements = null;
                        if (ResultCode.SUCCEEDED(m_session.Browse(((DaAddressSpaceElement)rootNode.Tag).ItemId, ((DaAddressSpaceElement)rootNode.Tag).ItemPath, browseOptions, out addressSpaceElements, m_executionOptions)))
                        {
		...
		private void itemsTreeView_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e) {
			try{
				...
				ListViewItem item;
                if (ResultCode.SUCCEEDED(m_session.Browse(((DaAddressSpaceElement)e.Node.Tag).ItemId, ((DaAddressSpaceElement)e.Node.Tag).ItemPath, browseOptions, out addressSpaceElements, m_executionOptions)))
                {
					for (int i = 0; i < addressSpaceElements.Length; i++) {

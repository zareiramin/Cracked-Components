...
//                            OPC TOOLBOX - C++                               |
//                                                                            |
//  Filename    : ServerDaAddressSpaceElement.cpp                             |
//  Version     : 4.31                                                        |
//  Date        : 01-August-2012                                              |
//                                                                            |
...
BOOL DaAddressSpaceElement::addChild(AddressSpaceElement* aChild)
{
	...
	if (pChild->getObjectHandle() == 0)
	{
		...
		data.m_name = (OTChar*)OTAllocateMemory((pChild->getName().size() + 1) * sizeof(OTChar));
		...
		data.m_itemID = (OTChar*)OTAllocateMemory((itemId.size() + 1) * sizeof(OTChar));
...
//-----------------------------------------------------------------------------
// Returns the string based addess space element
//
	...
	std::vector<ValueQT*>& aValueList)
{
	...
	long result = OTSValuesChanged(count, objects, values);
...
{
	unsigned long enumValuesCount = enumeratedValues.size();

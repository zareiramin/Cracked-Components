...
public:
		...
		OTSConditionDefinitionData& aDestination)
	{
		...
		unsigned long size = aSource.m_subconditions.size();
		...
		OTSConditionData& aDestination)
	{
		...
		aDestination.m_eventAttrCount   = aSource.m_eventAttributes.size();

		if (aDestination.m_eventAttrCount > 0)
		{
			aDestination.m_pEventAttrs = (OTVariant*)OTAllocateMemory(aSource.m_eventAttributes.size() * sizeof(OTVariant));
	...
	static long fillOTSEventData(AeEvent& aSource, OTSEventData& aDestination)
	{
		...
		unsigned long count = aSource.m_attributeValueList.size();
...
long Application::activate(
	...
	tstring aKey)
{
	return OTActivate((unsigned char)aFeature, (OTChar*)aKey.c_str(), 0);
}   //  end activate
...
long Application::changeConditions(std::vector<AeCondition*>& aConditionList)
{
	...
	unsigned long count = aConditionList.size();
...
{
	unsigned long count = anEventList.size();
...
void Application::removeSession(unsigned long aSessionHandle)
{
	...
	unsigned long result = m_sessionList.erase(aSessionHandle);
...
DaTransaction* Application::findTransaction(unsigned long aKey)
{
	...
	return transactionIterator->second;
}   //  end findTransaction
	...
	OUT OTSPropertyData** pPropertyList)
{
	...
	*pPropertyList = NULL;
	...
	{
		*aPropertyCount = propertyList.size();
		*pPropertyList = (OTSPropertyData*)OTAllocateMemory(propertyList.size() * sizeof(OTSPropertyData));
	...
	IN OTSSessionData* aSessionStateData)
{
	...
	case EnumSessionState_LOGON:
		...
		if (pSession != NULL)
		{
			...
			return pSession->logon(userName, sessionStateData);
		}   //  end if
	...
	OUT OTSValueData* pValue)
{
	...
	if (SUCCEEDED(result))
	{
		...
		{
			*(pValue->m_pValue) = *(valueData.getData());
		}
		else
		{
			pValue->m_pValue = (OTVariant*)(&valueData.getData());
		}   //  end if ... else
	}   //  end if
	...
	OUT OTChar***  pConditionNames)
{
	...
	*pConditionNames = NULL;
	...
	if (pObjectData != NULL)
	{
		...
		if (SUCCEEDED(result))
		{
			*pConditionCount = conditionNames.size();
	...
	OTSConditionDefinitionData* pConditionDefData)
{
	...
	return fillOTSConditionDefinitionData(*conditionDefinition, *pConditionDefData);
...
OTChar* OTSGlobals::allocateOTBString(tstring& aValue)
{
	...
	OTChar*  result = (OTChar*)OTAllocateMemory((aValue.size() + 1) * sizeof(OTChar));

...
long Application::activate(
	...
	tstring aKey)
{
	return OTActivate((unsigned char)aProduct, (OTChar*)aKey.c_str(), 0);
}   //  end activate
	...
	...)
{
	...
	Trace::writeline((unsigned char)aLevel, (unsigned char)aMask, anObjectIDStr, bufferStr);
}   // end Trace
		...
		if (daSession != NULL)
		{
			...
			if (daSubscription != NULL)
			{
				if (pItemData)
				{
					for (i = 0; i < count; i++)
					{
						DaItem* item = daSubscription->findItem(pItemData[i].m_userData);
						items[i] = item;
					}   //  end for
				}   //  end if
			...
			if (daSession != NULL)
			{
				...
				if (daSubscription != NULL)
				{
					if (pItemData)
					{
						for (i = 0; i < count; i++)
						{
							items[i] = daSubscription->findItem(pItemData[i].m_userData);
						} //end for
					}//end if
	...
	IN OTChar* reason)
{
	...
	if (daSession != NULL)
	{
		return daSession->onShutdown(tstring(reason));
	} //end if
	...
	if (aeSession != NULL)
	{
		return aeSession->onShutdown(tstring(reason));
	} //end if
		...
		if (session != NULL)
		{
			...
			if (subscription != NULL)
			{
				if (pItemData)
				{
					for (i = 0; i < itemCount; i++)
					{
						items[i] = subscription->findItem(pItemData[i].m_userData);
					} //end for
				}//end if

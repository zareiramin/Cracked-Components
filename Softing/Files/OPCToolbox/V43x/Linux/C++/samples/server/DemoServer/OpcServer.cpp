...
#endif
	getApp()->setDescription(_T("Softing OPC Toolbox C++ Demo Sample Server"));
...
long OpcServer::buildDAAddressSpace(void)
{
	...
	tag = creator->createBasicStaticElement(VT_BSTR);
	tstring chineaseName = _T("ä¸­å›½çš„");
	...
	tstring chPropName(_T("ä¸­å›½çš„"));
	chProperty->setName(chPropName);
	tstring chPropDescription(_T("ä¸­å›½çš„ Chinease"));
	...
	tag = creator->createBasicStaticElement(VT_BSTR);
	tstring japaneseName = _T("å¾¡å›½è¨€è‘‰");
	...
	tstring jpPropName(_T("å¾¡å›½è¨€è‘‰"));
	jpProperty->setName(jpPropName);
	tstring jpPropDescription(_T("å¾¡å›½è¨€è‘‰ Japanese"));

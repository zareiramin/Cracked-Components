...
void DaSession::setPerformInitialRead(BOOL performInitialRead)
{
	OTCObjectAttributes objectAttributes;
	memset(&objectAttributes, 0, sizeof(OTCObjectAttributes));
	objectAttributes.m_daSessionPerformInitialRead = performInitialRead;
	long setAttrsResult = OTCSetAttributes(
		getHandle(),
		(unsigned int)EnumObjectAttribute_DASESSION_PERFORMINITIALREAD,
		&objectAttributes);
	...
	ExecutionOptions* someExecutionOptions)
{
	...
	unsigned long idsCount = itemIds.size();
	unsigned long pathsCount = itemPaths.size();
	...
	ExecutionOptions* someExecutionOptions)
{
	...
	{
		unsigned long propertyIdsCount   = aGetPropertyOptions->getPropertyIds().size();
		unsigned long propertyNamesCount = aGetPropertyOptions->getPropertyNames().size();
